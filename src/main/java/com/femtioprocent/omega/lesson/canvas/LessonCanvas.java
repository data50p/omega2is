...
public class LessonCanvas extends BaseCanvas {
    ...
    public Manager om_msg_mgr = new Manager();
    static float gapB = 5;
...
// 	}
    ...
    public class Box {
        ...
        void draw(Graphics2D g2, boolean ink) {
            ...
            if (ss.length() > 1)
                ;//OmegaContext.sout_log.getLogger().info("ERR: " + "calc:   box:w=" + w + " string:w=" + sw + " string=" + ss + ' ' + ss.length());

            double d = wantRP;
            if (sw > w - gB / 2) {
                d = (double) sw / w;
                ;//OmegaContext.sout_log.getLogger().info("ERR: " + "this wRP " + d + " sw=" + sw + "  w = " + w + "   w-gB/2=" + (w-gB/2));
                if (d > wantRP) {
                    wantRP = d;
                }
            }
            ...
            if (wantRP >= 0.95 && wantRP <= 1.005) {
                if (ss.length() > 1)
                    ;//OmegaContext.sout_log.getLogger().info("ERR: " + "wantRP t' OK   " + wantRP + ' ' + d + ' ' + ss);
                g2.setFont(getItemFont());
                if (ink) {
                    g2.drawString(ss, (int) (x + 0.01 * getCaW()), y + (int) (h * 0.75));
                }
            } else if (ss.length() > 1)
                ;//OmegaContext.sout_log.getLogger().info("ERR: " + "wantRP t' ELS  " + wantRP + ' ' + d + ' ' + ss);


            if (edit) {
                int msk = getTarget().whatTargetMatchTid(itm.getEntryTid());
                int how_many = Num.howManyBits(msk);
...
// 	    }
    ...
    class AllBox {
        ...
        AllBox() {
            all = new HashMap();
//log	    OmegaContext.sout_log.getLogger().info("ERR: " + "AllBox CREATED " + new Date());
        }
...
// 	}
    ...
    private void gotoSameBox() {
        try {
            ...
            if (bx != null) {
                setQuitState("z3", 0);
//log		OmegaContext.sout_log.getLogger().info("ERR: " + "move " + current_ix + ' ' + current_iy + ' ' + nix + ' ' + niy);
                current_iy = niy;
                current_ix = nix;
                int[] where = new int[1];
                hitBox((int) (bx.r.getX()),
    ...
    public void addTarget(int where) {
        ...
        int sel = GetOption.getOption(T.t("Adding the target area"),
                sa);
        if (sel > 0) {
    ...
    public void delTarget(int where) {
        ...
        int sel = GetOption.getOption(T.t("Deleting the target area"),
                sa);
        if (sel > 0) {
    ...
    void addItemEntry(int ix) {
        ...
        int sel = GetOption.getOption(T.t("Add a new column"),
                sa);
        if (sel > 0) {
    ...
    public void delItemEntry(int ix) {
        ...
        int sel = GetOption.getOption(T.t("Delete selected column"),
                sa);
        if (sel > 0) {
...
//  		allbox.clearAll();
            ...
            for (int xi = 0; xi < nx; xi++) {
                ...
                //OmegaContext.sout_log.getLogger().info("ERR: " + "ratio " + xi + ' ' + ratio + ' ' + bx_w + ' ' + max_bxw + ' ' + sww + ' ' + gB);
...
//	show_action.setVisible(true);
    ...
    AnimAction anim_action_patch;

    public String waitReplyAction(AnimAction anim_action,
                                  String text,
                                  boolean show,
                                  Runnable myra) {
...
//	OmegaContext.sout_log.getLogger().info("ERR: " + "      >>>>>>>>     " + asp + ' ' + hh);
        return hh;
    }
...
//	    a.add(new Area(rt));
    ...
    void drawBoxes(Graphics2D g2) {
        synchronized (allbox_sy) {
            if (getAllBox() != null) {
                ...
                //OmegaContext.sout_log.getLogger().info("ERR: " + "wantRP is' " + wantRP);
    ...
    class MsgDialog {
        ...
        void draw(Graphics2D g2) {
            ...
            if (msg_item.image != null) {
                ...
                int ww = (4 * hh) / 3;
                try {
                    Image img = ScaledImageIcon.createImageIcon(LessonCanvas.this,
                            msg_item.image,
                            ww,
                            hh).getImage();
    ...
    public void setFrom(Element el, boolean dummy) {
        ...
        lesson_name = "-noname-";
//log	OmegaContext.sout_log.getLogger().info("ERR: " + "=-= dep_set lesson_name ");
...
//log	    OmegaContext.sout_log.getLogger().info("ERR: " + "=-= lesson_name is " + lesson_name);
        }
...
//log	    OmegaContext.sout_log.getLogger().info("ERR: " + "=-= lesson_link_next is " + lesson_link_next);
        }
